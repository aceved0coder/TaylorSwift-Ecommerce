    ...
    return (
        <div className='container mx-auto px-8 mt-2 min-h-screen'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2'>
                <div>
                    <p className='text-xl font-semibold text-myDarkColor'>Name: {user.first_name} {user.last_name}</p>
                    <p className='text-xl font-semibold text-myDarkColor'>Birthdate: {user.date_of_birth}</p>
                    <p className='text-xl font-semibold text-myDarkColor'>Email: {user.email}</p>
                </div>
                <div>
                    {user.documents && user.documents.length > 0 ? (
                        user.documents.map(doc => {
                            if (doc.name === 'profile') {
                                return <img key={doc._id} src={doc.reference} alt='Profile image' />;
                            }
                            return null;
                        })
                    ) : (
            ...
            <form onSubmit={handleSubmit} className='flex flex-col gap-5' encType="multipart/form-data">
                ...
                <p className='text-center text-l'>Sign out and sign back in to see the changes</p>
                <input type="submit" value="Upload" className='btn text-myBlue'/>
            </form>
        </div>
    )
}