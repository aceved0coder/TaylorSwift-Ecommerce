  ...
  return (
    <div className='container mx-auto mt-5'>
      <div className='w-full flex justify-end my-5 gap-5 text-myDarkColor'>
        <button className={`btn btn-sm ${searchParams.get('sort') === 'undefined' ? 'bg-myLightBlue' : ''}`} onClick={() => handlePriceSortChange('undefined')}>Default</button>
        <button className={`btn btn-sm ${searchParams.get('sort') === 'asc' ? 'bg-myLightBlue' : ''}`} onClick={() => handlePriceSortChange('asc')}>Lower Price</button>
        <button className={`btn btn-sm ${searchParams.get('sort') === 'desc' ? 'bg-myLightBlue' : ''}`} onClick={() => handlePriceSortChange('desc')}>Higher Price</button>
      </div>
